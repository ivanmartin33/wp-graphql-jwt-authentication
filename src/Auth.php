...
class Auth
{
	...
	public static function login_and_get_token($username, $password)
	{
		...
		/**
		 * If define GRAPHQL_JWT_AUTH_SET_COOKIES is true, set a X-Refresh cookie with refresh token httpOnly
		 */
		if (defined('GRAPHQL_JWT_AUTH_SET_COOKIES') && !empty(GRAPHQL_JWT_AUTH_SET_COOKIES) && GRAPHQL_JWT_AUTH_SET_COOKIES) {

			$refresh = self::get_refresh_token(wp_get_current_user());

			/**
			 * Front domain name for secure cookies
			 */
			$front_domain = defined('GRAPHQL_JWT_FRONT_DOMAIN') && !empty(GRAPHQL_JWT_FRONT_DOMAIN) ? GRAPHQL_JWT_FRONT_DOMAIN  : '';

			$arr_cookie_options = array(
				'expires' => apply_filters('graphql_jwt_auth_refresh_token_expiration', (self::get_token_issued() + (DAY_IN_SECONDS * self::$refresh_token_expiration))),
				// 'path' => '/',
				'domain' => $front_domain, // leading dot for compatibility or use subdomain
				'secure' => true,     // or false
				'httponly' => true,    // or false
				'samesite' => 'Strict' // None || Lax  || Strict
			);
			setcookie("X-Refresh", $refresh, $arr_cookie_options);
		}
